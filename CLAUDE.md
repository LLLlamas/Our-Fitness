...
| Medication pattern math ("usually logged around…") | `Domain/MedicationPattern.swift` → `recentDailyFirstLogs`/`typicalMinuteOfDay`/`hasDisplayablePattern`/`hasLogToday`/`nextFireDate` — pure, injectable now/calendar. Median of the earliest dose per day over 7 days, today excluded. Two thresholds, deliberately different: 1 day of history is enough to SCHEDULE a nudge, 2 (`minDaysForDisplay`) before the UI claims a routine. Build fire instants through calendar day+wall-clock components, never by adding minutes — that was an hour wrong on the spring-forward day. Tests: `MedicationPatternTests` |
| **Set** dose time vs **observed** one | `ReminderDTO.scheduledMinuteOfDay` (minutes from local midnight, optional, user-entered) beats `MedicationPattern.typicalMinuteOfDay` (inferred) everywhere both exist — resolved in ONE place, `MedicationPattern.effectiveMinuteOfDay`; never branch on it at a call site. A set time schedules from day one (no history needed); with none, everything behaves exactly as it did before the field existed. Minute-of-day ↔ Date conversions and all clock copy go through `clampMinuteOfDay`/`minuteOfDay(of:)`/`date(minuteOfDay:on:)`/`clockLabel` — a `DateComponents` on a fixed reference day, never a Date built from today (DST) |
| Dose logged vs its set time | `MedicationPattern.minutesFromScheduled` (signed; wraps to the NEAREST occurrence, so an 11pm dose logged at 00:20 is 80 min late, not 22 hr early) → copy via `timingLabel` ("12 min after 8:00 AM" / "On time" inside `onTimeToleranceMinutes`). Returns nil with no set time, which means render nothing |
...
| Reminders tab UI | `Features/Reminders/RemindersView.swift` + `AddReminderSheet.swift` + `ReminderDetailSheet.swift` + `LogMedicationSheet.swift` + `IntervalPicker.swift` + `AmountFlOzRow.swift`, `Components/ImagePickerView.swift`. `AddReminderSheet` deliberately has **no default group** (`resolvedGroupId` is nil until tapped) so Add doesn't drop the user into plant species search — don't reinstate a Plants fallback |
