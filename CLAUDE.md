...
| Apple Health "Move" card (3 columns) | `Features/Today/MoveCard.swift`. Columns: Apple Energy (HK daily sum), Training MET (sets+cardio+pilates only, steps excluded to avoid double-count with Apple active energy), Heart Rate. Data older than 7 days shows `"-"` / "need new data". Separate "as of" timestamp per column. ⓘ opens `MoveInfoSheet` explaining MET. Use `HealthKitService.latestHeartRateWithDate()` / `latestActiveEnergySampleDate()` for timestamped reads |
