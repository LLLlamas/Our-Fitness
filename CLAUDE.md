...
- **CI Xcode pin:** both workflows run `macos-26` / Xcode **26.0** (set via `XCODE_VERSION` env var at the top of each `.github/workflows/` file). Xcode 26 ships the **iphonesimulator26.0** SDK — never pin `-sdk iphonesimulator<version>` explicitly; let xcodebuild resolve the SDK from the selected Xcode. To upgrade Xcode, bump that var in both files together and also update the `runs-on` runner. `project.yml` no longer sets a hard `objectVersion` floor (removed when upgrading to Xcode 26); XcodeGen emits whatever format the installed Xcode needs. If you ever see *"cannot be opened because it is in a future Xcode project file format"*, the runner Xcode and any objectVersion pin are out of sync. Both the build step and the test step use a concrete simulator destination (`platform=iOS Simulator,name=iPhone 16,OS=26.0`) — **never `generic/platform=iOS Simulator`**. The build step also passes `ASSETCATALOG_COMPILER_SKIP_APP_STORE_DEPLOYMENT=YES` to suppress actool's SDK-version thinning pass. The test destination is pinned to `iPhone 16 (26.0)` / `OS=26.0`. If you bump Xcode, re-check which runtimes ship with the new runner image and update both destination pins in `compile.yml` and `fastlane/Fastfile` together.
