...
- **CI Xcode pin:** both workflows run `macos-15` / Xcode **16.3** (set via `XCODE_VERSION` env var at the top of each `.github/workflows/` file). To upgrade Xcode, bump that var and `runs-on` in both files together. `project.yml` also sets `objectVersion: "56"` — this is a hard floor that prevents XcodeGen from emitting a project format newer than what the pinned Xcode can read. If you ever see *"cannot be opened because it is in a future Xcode project file format"*, the runner Xcode and this objectVersion are out of sync. The build step passes `ASSETCATALOG_COMPILER_SKIP_APP_STORE_DEPLOYMENT=YES` to suppress actool's simulator-runtime thinning pass — without it, actool errors when no installed simulator runtime matches the SDK version. The test destination is pinned to `iPhone 16 (18.5)` / `OS=18.5` (runtime `22F77`) because `macos-15` runners with Xcode 16.3 ship SDK 18.4 but only install the 18.5+ runtimes; using `OS=latest` or `OS=18.4` causes a hard SDK↔runtime mismatch. If you bump Xcode, re-check which runtimes ship with the new runner image and update the destination pin in `compile.yml` and `fastlane/Fastfile` together.
