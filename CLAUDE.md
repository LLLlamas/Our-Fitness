...
- Bootstrap or rotation only: dispatch with `refresh_signing` ✅ checked → `MATCH_READONLY=false` → `fastlane ios sync_signing` runs before XcodeGen/tests so cert-capacity failures fail fast. **This also passes `force: true` to match** (see `Fastfile.match_options`) so the provisioning profile is regenerated against the current App ID capability set. Without `force`, match silently reuses the cached profile and any newly-enabled capability (HealthKit, Background Delivery, Push, etc.) appears on device as `Missing com.apple.developer.<x> entitlement` even after a fresh build.
- After enabling any new capability in the Apple Developer portal App ID, you MUST run TestFlight once with `refresh_signing` checked. The capability change only takes effect once a regenerated profile is signed into the IPA.
